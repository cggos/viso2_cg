...
inline void Matcher::computeDescriptor (
        const uint8_t *I_du,
        const uint8_t *I_dv,
        const int32_t &bpl,
        const int32_t &u,
...
void Matcher::computeDescriptors (
        uint8_t* I_du,
        uint8_t* I_dv,
        const int32_t bpl,
...
void Matcher::computeFeatures (
    ...
    num1 = (int32_t)maxima1.size();
    num2 = (int32_t)maxima2.size();
    max1 = 0;
    max2 = 0;
...
void Matcher::matching (
        int32_t *m1p, int32_t *m2p,
        int32_t *m1c, int32_t *m2c,
        int32_t  n1p, int32_t  n2p,
        int32_t  n1c, int32_t  n2c,
    ...
    if (method == 0) {
        ...
        for (i1p = 0; i1p < n1p; i1p++) {
            ...
            if (Tr_delta) {
                ...
                findMatch(m2p, i2p, m2c, step_size, k2c, u_bin_num, v_bin_num,
                          stat_bin, i2c, 1, true, use_prior, u2c_, v2c_);
            } else {
            ...
            if (Tr_delta)
                findMatch(m1c, i1c, m1p, step_size, k1p, u_bin_num, v_bin_num,
                          stat_bin, i1p2, 3, true, use_prior, u1p, v1p);
            else
...
void Matcher::removeOutliers (vector<Matcher::p_match> &p_matched,int32_t method) {
    ...
    for (int32_t i = 0; i < out.numberoftriangles; i++) {
        ...
        if (method == 0) {
            ...
            // consistency of 1. edge
            if (fabs(p1_disp - p2_disp) < param.outlier_disp_tolerance &&
                fabs(p1_flow_u - p2_flow_u) + fabs(p1_flow_v - p2_flow_v) < param.outlier_flow_tolerance) {
                num_support[p1]++;
                num_support[p2]++;
            }
            ...
            // consistency of 2. edge
            if (fabs(p2_disp - p3_disp) < param.outlier_disp_tolerance &&
                fabs(p2_flow_u - p3_flow_u) + fabs(p2_flow_v - p3_flow_v) < param.outlier_flow_tolerance) {
                num_support[p2]++;
                num_support[p3]++;
            }
            ...
            // consistency of 3. edge
            if (fabs(p1_disp - p3_disp) < param.outlier_disp_tolerance &&
                fabs(p1_flow_u - p3_flow_u) + fabs(p1_flow_v - p3_flow_v) < param.outlier_flow_tolerance) {
                num_support[p1]++;
                num_support[p3]++;
            }
        }
